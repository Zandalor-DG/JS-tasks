...
function digToText(dig) {
    ...
    const words = {
        ...
        m1: [
            "дестять", "двадцать", "тридцать", "сорок", "пятьдесят", "шестьдесят", "семьдесят", "восемдесят",
            "девяносто"
        ],
        m0: ["один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять", "десять"],
        f0: ["одна", "две"],
        l0: [
            "дестять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать", "шестнадцать",
            "семнадцать", "восемнадцать", "девятнадцать"
        ]
    };
    const dim = function(dig, power, words) {
        result = "";
        const pow = Math.floor(dig / Math.pow(10, power)) % Math.pow(10, 3);
        if (!pow) {
            return result;
        }
        const n2 = Math.floor(pow / 100);
        const n1 = Math.floor(pow % Math.pow(10, 2) / 10);
        const n0 = Math.floor(pow % 10);
        const s1 = (n1 > 0) ? " " : "";
        const s0 = (n0 > 0) ? " " : "";
        const get_n = function() {
            switch (power) {
                case 0:
                case 6:
                case 9:
